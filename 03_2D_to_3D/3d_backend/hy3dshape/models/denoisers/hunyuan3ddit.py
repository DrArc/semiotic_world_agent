    ...
    def __init__(
        ...
        **kwargs,
        ...
        if sum(axes_dim) != pe_dim:
            # Auto-adjust axes_dim to match pe_dim if there's a mismatch
            if len(axes_dim) == 1 and axes_dim[0] == 64 and pe_dim == 128:
                axes_dim = [128]  # Common case: double the dimension
            elif len(axes_dim) == 1 and pe_dim == 128:
                # More general case: scale axes_dim to match pe_dim
                scale_factor = pe_dim // axes_dim[0]
                axes_dim = [axes_dim[0] * scale_factor]
            else:
                # Last resort: just use pe_dim as the axes_dim
                axes_dim = [pe_dim]
        self.hidden_size = hidden_size
        self.num_heads = num_heads
...
# Alias for compatibility with model configurations
try:
    HunYuanDiTPlain
except NameError:
    # Pick the appropriate existing class in this file:
    # e.g., HunyuanDiTPlain or Hunyuan3DDiTPlain — alias it:
    HunYuanDiTPlain = Hunyuan3DDiT
